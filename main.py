...
def main() -> None:
    ...
    def cmd_toeic_corpus(_: argparse.Namespace) -> None:
        from toeic800.processing.toeic_practice import corpus_stats

        s = corpus_stats()
        print("TOEIC 800-900 RAG corpus (original, non-news):")
        for k, v in s.items():
            print(f"  {k}: {v}")
        print(f"  total: {sum(s.values())}")

    sub.add_parser("toeic-corpus").set_defaults(func=cmd_toeic_corpus)
